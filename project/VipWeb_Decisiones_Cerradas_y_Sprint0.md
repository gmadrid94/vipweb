...
Las decisiones fundacionales quedan cerradas para Sprint 0. ADR-030 incorpora el modelo AI-first con Git local y tools externas sin abrir desarrollo visible.

## Corrección crítica aplicada

El proyecto se desarrolla actualmente en **Windows con WAMP, local, Git en el repo oficial para AI-first y sin despliegue externo**. Por tanto:

- no se habla de producción real en esta fase;
- no se exige CI/CD;
- no se exige staging;
- rollback significa restaurar carpeta + SQL;
- Git aporta diff y baseline, pero no sustituye backups ni evidencia;
- `evidence/sprint-0-local-execution.md` conserva el registro manual validado;
- Doctor/validador y evidencias se ejecutan localmente;
- las tools externas generan logs en `C:/wamp64/www/vipweb_tools/logs`.
...
- **ADR-021** — Entorno local Windows/WAMP y recuperación manual: WAMP local sin despliegue; backups + SQL + Doctor + registro de ejecución local.
