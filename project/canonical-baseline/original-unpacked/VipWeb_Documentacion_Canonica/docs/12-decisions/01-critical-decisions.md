...
| ADR-021 | Entorno local Windows/WAMP y recuperación manual | WAMP local, sin Git ni despliegue; backups + SQL + Doctor + registro de ejecución local. | Sí |
...
No queda ninguna decisión fundacional abierta antes de Sprint 0. La única condición operativa imprescindible es respetar ADR-021: trabajar en local WAMP con backup, exportación SQL, Doctor, evidencias y `evidence/sprint-0-local-execution.md`.
