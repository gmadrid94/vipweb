...
1. **validación documental CLI:** `scripts/update_script.py`, ya implementado y ejecutable sobre `PROJECT_CONTROL_ROOT`.
...
5. Se ejecuta `python project/scripts/update_script.py` desde `APP_ROOT` o `python scripts/update_script.py` desde `PROJECT_CONTROL_ROOT` y su salida se registra manualmente si se usa como evidencia.
