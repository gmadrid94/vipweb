...
## Regla de rutas Sprint 0

`APP_ROOT` es `C:/wamp64/www/vipweb`. `PROJECT_CONTROL_ROOT` es `C:/wamp64/www/vipweb/project`. `BACKUP_ROOT` es `C:/wamp64/www/vipweb_backups`. `EXTERNAL_ROADMAP_TOOLS_ROOT` es `C:/wamp64/www/vipweb_tools`. Salvo indicación contraria, `docs/`, `machine/`, `database/`, `scripts/`, `templates/` y `evidence/` son relativos a `PROJECT_CONTROL_ROOT`.



## Modelo de ejecución AI-first para Sprint 0

Sprint 0 debe ejecutarse preferentemente mediante tools externas cuando una tarea requiera acceso local real. Las tools externas viven en `C:/wamp64/www/vipweb_tools`, generan logs en `C:/wamp64/www/vipweb_tools/logs` y no sustituyen la evidencia manual.

`C:/wamp64/www/vipweb/tools` queda reservado para tools internas del producto. No sustituye a la carpeta externa `C:/wamp64/www/vipweb_tools`.

El owner ejecuta los `.bat/.cmd`, pasa el log más reciente a GPT y registra manualmente la evidencia validada en `evidence/sprint-0-local-execution.md`.


## Decisión sobre scripts y Doctor

El único validador documental ejecutable de esta baseline es `scripts/update_script.py` bajo `PROJECT_CONTROL_ROOT`. Doctor queda definido como contrato runtime futuro mediante `machine/doctor_checks.json`.
...
Registrar versiones reales de PHP, Apache/WAMP y MySQL/MariaDB, verificar que WAMP sirve `C:/wamp64/www/vipweb/public` y confirmar que carpetas privadas no son públicas antes de escribir código runtime.
...
- Ejecutar la tool externa `S0-000-check-wamp-env` desde `C:/wamp64/www/vipweb_tools` si está disponible
- Ejecutar php -v
...
- Verificar que WAMP sirve `C:/wamp64/www/vipweb/public` como public root
- Verificar que carpetas privadas no son accesibles por navegador
- Guardar log externo en `C:/wamp64/www/vipweb_tools/logs`
...
- `PUBLIC_DOCUMENT_ROOT = C:/wamp64/www/vipweb/public` verificado
- Carpetas privadas no accesibles por navegador
- Log externo de S0-000 guardado y revisado
...
- wamp-runtime-version-report + public-root-exposure-report + external-tool-log-ref
...
- Fase: `Sprint 0`
- Estado: `pending`
- Doctor check: `PROJECT_CLEAN_ROOT_LAYOUT_DECLARED`
- Bloqueante: `true`
- Relevante WAMP: `true`
- Lectura IA requerida: `true`
- Fuente: `docs/11-backlog/06-sprint-0-ejecutable.md`
...
Crear esqueleto del proyecto con raíz runtime limpia y raíz documental/de control bajo `project/`.
...
- Crear `APP_ROOT/public`
- Crear `APP_ROOT/app`
- Crear `APP_ROOT/config`
- Crear `APP_ROOT/project`
- Crear `APP_ROOT/tools`
- Crear `APP_ROOT/logs`
- Crear `APP_ROOT/tmp`
- Confirmar que `PROJECT_CONTROL_ROOT` contiene `docs/`, `machine/`, `database/`, `scripts/`, `templates/`, `evidence/`, `changelog/` y `canonical-baseline/`
- Confirmar que `docs/`, `machine/`, `database/`, `scripts/`, `templates/` y `evidence/` no quedan duplicados en `APP_ROOT`
...
- `APP_ROOT/logs`
- `APP_ROOT/tmp`
- `PROJECT_CONTROL_ROOT/changelog`
- `PROJECT_CONTROL_ROOT/canonical-baseline`

### Criterios de aceptación

- Todas las carpetas existen
- `PROJECT_CLEAN_ROOT_LAYOUT_DECLARED` pasa
- La raíz local no contiene `docs/`, `machine/`, `database/`, `scripts/`, `templates/` ni `evidence/` fuera de `project/`
- `public/` queda identificado como único punto visible
...
- Desarrollo desordenado o documentación repartida en la raíz runtime
...
- Sincronizar machine/decisions.json incluyendo ADR-029
...
- Existen 28 ADRs aceptadas: ADR-001 a ADR-025, ADR-027, ADR-028 y ADR-029
...
Conservar `scripts/update_script.py` bajo `PROJECT_CONTROL_ROOT` como único validador documental ejecutable de esta baseline. Doctor queda definido como contrato runtime futuro mediante `machine/doctor_checks.json`.
...
- Conservar scripts/update_script.py como único script ejecutable de validación documental actual bajo PROJECT_CONTROL_ROOT
...
- Existe scripts/update_script.py y es ejecutable con python scripts/update_script.py desde PROJECT_CONTROL_ROOT y con python project/scripts/update_script.py desde APP_ROOT
...
- stdout del validador readonly y registro manual en evidence/sprint-0-local-execution.md
...
Ejecutar scripts/update_script.py como validador documental readonly bajo PROJECT_CONTROL_ROOT. Esta ejecución no modifica archivos ni genera evidencia canónica automática.
...
- Ejecutar python project/scripts/update_script.py desde APP_ROOT o python scripts/update_script.py desde PROJECT_CONTROL_ROOT
...
- El validador readonly finaliza con status passed
...
Completar evidence/sprint-0-local-execution.md bajo PROJECT_CONTROL_ROOT con evidencia local real de Sprint 0. scripts/update_script.py solo valida y no genera ni actualiza evidencia.
...
- Ejecutar python project/scripts/update_script.py desde APP_ROOT o python scripts/update_script.py desde PROJECT_CONTROL_ROOT como validación readonly previa o posterior
...
- `python project/scripts/update_script.py` desde APP_ROOT o `python scripts/update_script.py` desde PROJECT_CONTROL_ROOT pasa sin fallos.
...
- salida del validador readonly revisada y registrada manualmente si se usa como evidencia;
