...
**Entorno vigente:** Windows local + WAMP + Git local en el repo oficial para AI-first + sin despliegue externo.  
...
- Decisiones bloqueantes cerradas: **25**.
- Decisiones no bloqueantes pero vinculantes para MVP/UX/IA: **4**.
- Corrección crítica incorporada: **ADR-021 adapta operación a Windows/WAMP local; ADR-030 añade ejecución AI-first, Git local en el repo oficial y tools externas sin despliegue externo**.
...
| ADR-029 | Raíz limpia y separación entre runtime root y project control root | Operación local/Documentación | Aceptada | Sí | ADR-007, ADR-021, ADR-024 |
| ADR-030 | Modelo de ejecución AI-first con tools externas locales | IA/Operación local/Roadmap | Aceptada | Sí | ADR-003, ADR-005, ADR-007, ADR-009, ADR-021, ADR-024, ADR-026, ADR-029 |
...
Todo archivo dentro de `docs/12-decisions/adr/` cuenta como ADR aceptada. ADR-029 fija el layout `APP_ROOT` / `PROJECT_CONTROL_ROOT`; ADR-030 fija el modelo AI-first y las raíces externas de la baseline 1.2. Toda ADR propuesta o condicional debe vivir en `docs/12-decisions/proposed/` y declarar metadata `status` y `counts_as_accepted_adr=false`.
