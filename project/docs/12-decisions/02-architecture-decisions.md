...
| ADR-021 | Entorno local Windows/WAMP y recuperación manual | VipWeb se desarrollará inicialmente en Windows con WAMP y sin despliegue externo. La protección operativa será mediante copias completas, exportaciones SQL, Git local cuando aplique, Doctor, evidencias y evidence/sprint-0-local-execution.md. |
...
| ADR-030 | Modelo de ejecución AI-first con tools externas locales | El roadmap se ejecuta con GPT + Codex + tools externas locales, Git en el repo oficial, backups externos y logs revisables por GPT. |
